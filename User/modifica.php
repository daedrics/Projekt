...
$kontrata_data=mysqli_query($link,"SELECT * FROM `draft` WHERE `id`='$id_kontrata'");
...
<div id="wrapper" >
    ...
    if(isset($_POST['cambia'])){
        ...
        if($r1!=0){
            if($nuovo_pass==$conf_pass){
                $sql3=mysqli_query($link,"UPDATE `operator` SET `password` = '$nuovo_pass' WHERE `admin`.`id` = '$pid'");
                echo '<script language="javascript">';
    ...
    <div id="page-wrapper">
        ...
        <div class="row">
        <form method="post" class="text-primary" id="form1">
            <div class="row">
            ...
            <div class="row">
                ...
                <div class="form-group col-lg-2">
                    <label>Stato</label>
                    <input type="text" class="form-control" name="stato" value="<?php echo $info1[5];?>" readonly>
                </div>
            ...
            <div class="row">
                <div class="form-group col-lg-12">
                    ...
                    <textarea  class="form-control" name="motivazione"  style="height: 100px" readonly> <?php echo $info1[29];?></textarea>
                </div>
            </div>
            <div class="row">
                <div class="form-group col-lg-12">
            ...
            <button type="submit" name="insert" class="btn btn-success">Inscerisci Contratto</button>
...
UPDATE `draft` SET `emer` = '$info[11]', `mbiemer` = '$info[12]', `status` = '$info[26]', `gestore_tel` = '$info[1]', `tipologia_cnt` = '$info[2]', `app_cnt` = '$info[3]', 
...
if(isset($_POST['elimina'])){
    ...
    //shto ne databaze
    $query = mysqli_query($link,"
DELETE FROM `kliente`  WHERE `kliente`.`id` = '$id_kontrata'
...
if(isset($_POST['insert'])){
    $id=$_POST['id'];
    $kontrata1= mysqli_query($link,"SELECT * FROM `draft` WHERE `id`='$id_kontrata'");
...
INSERT INTO `kliente` (`id`, `data`, `emer`, `mbiemer`, `status`, `#id_Operator`, `gestore_tel`, `tipologia_cnt`, `app_cnt`, `numero_fisso`, `comune`, `provincia`, `cap`,
 ...
 `recapito_cell`, `operatore_cell`, `offerta_scelta`, `cell_off_tsm`, `iccid`, `codice_op`, `motivazione`, `note`, `id_admin`, `frazione`, `id_backoffice`)
  ...
  '$info1[23]','$info1[24]','$info1[25]','$info1[26]','$info1[27]','$info1[28]','$info1[29]','$info1[30]','' ,'$info1[12]','') ;");
    ...
    $query = mysqli_query($link,"
DELETE FROM `draft`  WHERE `draft`.`id` = '$id_kontrata'");

    echo '<script language="javascript">';
    echo 'alert("Il contratto e stato inserito \n")';
    echo '</script>';
    echo "<script> location.href='arkiv.php'; </script>";
